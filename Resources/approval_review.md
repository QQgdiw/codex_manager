...
| `mcp.modelcontextprotocol.filesystem` | mcp | proposed | needs_review | high | 文件系统 MCP 访问 | 涉及较广的文件读写能力，批准前需要明确根目录、写入范围和回滚边界。 |
...
| `skill.context-engineering.filesystem-context` | skill | proposed | needs_review | medium | 基于文件系统的上下文收集 | 批准前需要复核数据最小化要求，避免过度读取工作区文件。 |
