...
2026-07-15 已进入市场文档质量审计与整理阶段，重点覆盖 `plugins_market.md`、`github_market.md`、`MCP_market.md` 和 `tool_market.md`。当前环境的 Plugins 完整目录必须以 app-server `plugin/list` 为准；旧 `openai/plugins.git` 和 `codex plugin list` 只覆盖不同范围的 snapshot，不能继续作为 `/plugins` 完整清单。用户已审阅并确认设计与实施计划。Task 1 已完成 Plugins 目录采集器、真实 Codex app-server envelope 兼容、Windows npm shim 安全启动、严格请求生命周期验证，以及对真实 `plugin.interface` 嵌套元数据和研发者重点索引的渲染；Task 2 已更新 PRD、PP、GUIDE 和历史设计边界，并建立可阻止旧来源回退的合同测试。Task 3 已原子更新 `Resources/plugins_market.md`：同次正式采集得到 1 个 marketplace、2,034 条记录和 1,047 条可回溯重点索引，且即时检查成功。Task 4 已实现 GitHub 候选采集与跨文档验证器：采集请求与结果均限制 `stars >= 1000`，失败不覆盖目标文件，派生记录可验证来源与 MCP/tool 冲突；其余三份市场正文尚未改写。

2026-07-15 用户确认 W01-W26 也必须按新规则逐项重审。`docs/superpowers/specs/2026-07-15-github-market-historical-rebuild-design.md` 已定义 W01-W29 的历史重建边界，当前等待用户审阅设计文档；在此之前不得改写 `github_market.md`。
