...
- [x] 用户审阅并确认事件市场重建实施计划。
- [x] Task 1：实现事件总账验证器、原子渲染器和结构测试；Node 23/23、Pester 6/6 通过，最终独立复审无高、中问题。
- [x] Tasks 2-4：分领域调研并生成 64 条临时候选；三个总账均通过验证，跨文件无重复 ID、来源 URL 或 keep `mergeKey`。
- [x] Task 5：主代理打开 64 个原始来源及 4 个辅助来源，合并并裁决为 34 keep、35 exclude；正式总账验证通过。
- [ ] Task 6：渲染候选并完成事实与中文质量两轮审阅（执行中）。
- [ ] Task 7：提升正式文档并更新 GUIDE/PP。
- [ ] Task 8：全量验证、状态关闭、提交和推送。
