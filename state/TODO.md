...
- [x] 用户审阅并确认事件市场重建实施计划。
- [x] Task 1：实现事件总账验证器、原子渲染器和结构测试；最终 Node 31/31、Pester 6/6 通过，独立复审无高、中问题。
- [x] Tasks 2-4：分领域调研并生成 64 条临时候选；三个总账均通过验证，跨文件无重复 ID、来源 URL 或 keep `mergeKey`。
- [x] Task 5：主代理打开 64 个原始来源及 4 个辅助来源，合并并裁决为最终 33 keep、36 exclude；正式总账验证通过。
- [x] Task 6：渲染候选并完成事实、简体中文和代码质量独立审阅；最终无高、中问题。
- [x] Task 7：原子提升正式文档并更新 `GUIDE.md`、`PP.md`；专项文档复审无高、中问题。
- [x] Task 8：全量验证、最终独立审阅和状态关闭均已完成；等待提交和推送。
- [x] 完成候选调研、逐项核验、正式文档重建和最终质量审计。
